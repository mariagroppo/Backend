...
const viewProducts = async (req, res) => {
    ...
    try {
        ...
        let listado = await productService.getAll(validLimit,validPage,sort,category,userId,enabled,edition, purchase);
        ...
        if (listado.value.docs.length>0){
            res.render('../src/views/main.hbs', {prods: listado.value.docs, productsExists: true, realTime: false, hasPrevPage, hasNextPage, prevPage, nextPage, validPage, validLimit, sort, category, userStatus: true, userName, enabled, edition,purchase })            
        } else {
            res.render('../src/views/main.hbs', {prods: listado.value, productsExists: false, realTime: false, userStatus: true, userName, enabled, edition, purchase})
